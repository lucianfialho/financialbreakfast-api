...
-- Create transcription segments table WITHOUT vector embeddings (fallback)
CREATE TABLE IF NOT EXISTS call_segments (
    ...
    -- embedding vector(768), -- REMOVED: Requires pgvector extension
...
-- Create full-text search index (works without pgvector)
...
COMMENT ON TABLE call_segments IS 'Stores transcribed segments for text search (vector embeddings disabled)';
